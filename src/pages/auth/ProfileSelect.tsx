...
export const ProfileSelect: React.FC = () => {
  ...
  return (
    <div style={{
      ...
      <div style={{ zIndex: 10, maxWidth: 800, margin: '0 auto', width: '100%', textAlign: 'center', marginTop: '2rem' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '1rem', background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(10px)', padding: '1rem 2rem', borderRadius: '2rem', marginBottom: '3rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '2rem' }}>
          {profiles.map(p => (
            ...
            >
              {p.avatar ? (
                <img src={p.avatar} alt={p.name} style={{ width: 80, height: 80, borderRadius: '50%' }} />
              ) : (
                <UserCircle size={80} color="#4f46e5" />
              )}
              <span style={{ fontSize: '1.25rem', fontWeight: 600, color: '#1e293b' }}>{p.name}</span>
            </button>
          ))}
        </div>
      </div>
